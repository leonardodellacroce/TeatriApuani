...
export default function NewCompanyPage() {
  ...
  const validate = () => {
    ...
    if (!formData.codiceSDI || formData.codiceSDI.trim() === "" || !/^[A-Z0-9]{7}$/.test(formData.codiceSDI)) {
      setError("Il codice SDI è obbligatorio e deve essere esattamente 7 caratteri");
      return false;
    }
    // Email: deve contenere @ esattamente una volta
    ...
    // PEC: deve contenere @ esattamente una volta
    if (!formData.pec || formData.pec.trim() === "" || !/^[^@]+@[^@]+$/.test(formData.pec)) {
      setError("La PEC è obbligatoria e deve essere un indirizzo valido");
      return false;
    }
