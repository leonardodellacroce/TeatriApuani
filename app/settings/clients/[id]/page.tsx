...
export default function EditClientPage() {
  ...
  const validateField = (name: string, value: string): boolean => {
    // Provincia: solo lettere maiuscole
  ...
  const validate = () => {
    if (!clientType) {
      setError("Il tipo cliente è obbligatorio");
      return false;
    }
    ...
    if (clientType === "AZIENDA") {
      if (formData.email && formData.email.trim() !== "" && !/^[^@]+@[^@]+$/.test(formData.email)) {
        setError("L'email deve essere un indirizzo valido");
        return false;
      }
    }
      ...
      <div>
        ...
        <form onSubmit={handleSubmit} className="max-w-2xl space-y-4">
          ...
          <div>
            ...
            <p className="text-sm text-gray-500 mt-2">Il tipo cliente non può essere modificato</p>
          </div>
          ...
          <div className="flex gap-4 pt-4">
            <button
              type="submit"
              disabled={saving || !clientType || invalidFields.size > 0}
        ...
        <ConfirmEditDialog
          ...
          title="Conferma Modifiche Cliente"
