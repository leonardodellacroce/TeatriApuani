...
export default function EditDocumentTemplatePage() {
  ...
  }, [isAdmin, status, router, templateId]);
  ...
  const fetchTemplate = async () => {
    try {
      const res = await fetch(`/api/doc-templates/${templateId}`);
      if (res.ok) {
        const data = await res.json();
        const parsedPages = JSON.parse(data.blocksJson);
        
        // Gestisci retrocompatibilità: se blocksJson è un array di blocchi, convertilo in pagine
        let pages: Page[];
        if (Array.isArray(parsedPages) && parsedPages.length > 0 && !parsedPages[0].hasOwnProperty('id')) {
          // È un vecchio formato con array di blocchi
          pages = [{
            id: crypto.randomUUID(),
            blocks: parsedPages
          }];
        ...
        setTemplate({
          title: data.title,
          pageSettings: JSON.parse(data.pageSettings),
          pages: pages,
        });
        setCategory(data.category || "");
        setTemplateLocationId(data.locationId || "");
      } else {
  ...
  const handleSave = async () => {
    try {
      console.log("Aggiornamento template:", template);
      ...
      if (res.ok) {
        const data = await res.json();
        console.log("Template aggiornato:", data);
        setHasUnsavedChanges(false);
        router.push("/dashboard/advanced-locations/document-templates");
      } else {
        const errorText = await res.text();
        console.error("Errore aggiornamento (status", res.status, "):", errorText);
        try {
          const errorJson = JSON.parse(errorText);
          console.error("Error details:", errorJson);
        } catch {}
        alert("Errore durante l'aggiornamento del template: " + errorText);
      }
    } catch (error) {
      console.error("Error updating template:", error);
      alert("Errore durante l'aggiornamento del template");
    }
  };
